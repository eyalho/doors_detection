...
class IMAGE:
    ...
    def show_ROI(self, title, edgecolor, numGT, text, saveDir=None):
        ...
        for i in range(numGT, len(self.ROIS)):
            ...
            ax.add_patch(rect)
        if (saveDir is None):
            ax.text(15, 160, text, fontdict=fontdict,
                    bbox={'facecolor': 'yellow', 'edgecolor': 'yellow', 'alpha': 0.5, 'pad': 2})
        else:
            ax.text(15, 300, text, fontdict=fontdict,
                    bbox={'facecolor': 'yellow', 'edgecolor': 'yellow', 'alpha': 0.5, 'pad': 2})
        plt.title(title)
...
def runTest(annFileNameGT, myAnnFileName, busDir, saveDir=None, elapsed=None):
    ...
    FP = 0
    MISS = 0
    print(len(writtenAnnsLines['Ground_Truth']))
    ...
    strToWrite = 'Total detections = {}/{}\nTotal False Positives = {}\nTotal missed = {}'.format(TP, TP + MISS, FP,
                                                                                                  MISS)
...
if __name__ == "__main__":
    ...
    # myAnnFileName = os.path.join("doors", "annotations_with_loaded_model.txt")
    ...
    runTest(annFileNameGT, myAnnFileName, busDir, saveDir=saveDir, elapsed=elapsed)
